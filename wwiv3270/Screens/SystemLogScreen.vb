...
Namespace WWIV.Screens
    ...
    Public Class SystemLogScreen
        ...
        Private Sub RenderTelnet(session As ISession)
            Dim logLines = Logger.GetLogLines(20)
            session.ClearScreen()
            session.WriteLine(Util.Ansi.Color(Util.Ansi.White, Util.Ansi.BgRed) & " WWIV System Log (Last 20 Events) ".PadRight(70) & Util.Ansi.Reset)
            session.WriteLine("")
            For Each line In logLines
                session.WriteLine(Util.Ansi.Color(Util.Ansi.White) & line & Util.Ansi.Reset)
            Next
            If logLines.Count = 0 Then session.WriteLine(Util.Ansi.Color(Util.Ansi.Yellow) & "(No events logged)" & Util.Ansi.Reset)
            session.WriteLine("")
